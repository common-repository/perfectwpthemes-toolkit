...
if ( ! class_exists( 'Perfectwpthemes_Toolkit_Theme_Info' ) ) {
	...
	class Perfectwpthemes_Toolkit_Theme_Info {
		 ...
		 * Plugin version.
		 *
		 * @var string $plugin_version Theme version.
		 */
		...
		public function setup_config() {
			...
			$this->plugin_version = isset( $this->config['plugin_version'] ) ? $this->config['plugin_version'] : '';

			$this->theme_slug    = $theme->get_template();

			$this->plugin_slug   = isset( $this->config['plugin_name'] ) ? $this->config['plugin_name'] : '';
			
			if( $this->plugin_slug ) {
				
				$this->page_slug     = $this->plugin_slug . '-about';
			} else {
		...
		public function render_about_page() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				if( $this->plugin_version || $this->theme_version ) {
					?>
					<div class="pt-version">
						<p><b><span><?php echo __( 'Current Version: ', 'perfectwpthemes-toolkit' ); ?></span><?php if( $this->plugin_version ) { echo esc_html( $this->plugin_version ); } else { echo esc_html( $this->theme_version ); } ?></b></p>
					</div>
					<?php
				}

				if ( ! empty( $welcome_content ) ) {
					echo '<div class="about-text"><p>' . wp_kses_post( $welcome_content ) . '</p></div>';
				}
		...
		public function getting_started() {
			...
			if ( ! empty( $this->config['getting_started'] ) ) {
				...
				if ( ! empty( $getting_started ) ) {
					...
					echo '<div class="pt-row pt-col-3">';

					foreach ( $getting_started as $getting_started_item ) {

						echo '<div class="pt-col">';

						echo '<div class="pt-col-element">';
